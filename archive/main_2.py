...
def open_menu(menu_type, name, balance):
    ...
    def shot_button_pressed(button):
        ...
        if last_shot_button:
            last_shot_button.config(bg="SystemButtonFace")  # Reset color of the last pressed button
        last_shot_button = button
        button.config(bg="green")  # Change color of the current button to green
    ...
    def amount_button_pressed(button):
        ...
        if last_amount_button:
            last_amount_button.config(bg="SystemButtonFace")  # Reset color of the last pressed button
        last_amount_button = button
        button.config(bg="blue")  # Change color of the current button to blue
    ...
    name_entry = tk.Entry(menu_window, state='readonly', textvariable=tk.StringVar(menu_window, value=name))
    name_entry.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    balance_label = tk.Label(menu_window, text="Balance:")
    balance_label.grid(row=0, column=2, padx=5, pady=5, sticky="e")
    balance_entry = tk.Entry(menu_window, state='readonly', textvariable=tk.StringVar(menu_window, value=balance))
    balance_entry.grid(row=0, column=3, padx=5, pady=5, sticky="w")

    # Populate the "Drink" field with the name of the menu
    drink_label = tk.Label(menu_window, text="Drink:")
    drink_label.grid(row=1, column=0, padx=5, pady=5, sticky="e")
    drink_entry = tk.Entry(menu_window, state='readonly')
    ...
    price_label = tk.Label(menu_window, text="Price:")
    price_label.grid(row=1, column=2, padx=5, pady=5, sticky="e")
    price_entry = tk.Entry(menu_window, state='readonly')
    ...
    amount_label = tk.Label(menu_window, text="Amount (oz):")
    ...
    percentage_label = tk.Label(menu_window, text="Alcohol Percentage:")
    percentage_label.grid(row=4, column=0, padx=5, pady=5, sticky="e")
    percentage_entry = tk.Entry(menu_window, state='readonly')
    percentage_entry.grid(row=4, column=1, padx=5, pady=5, sticky="w")

    # Create button to dispense drink
    dispense_button = tk.Button(menu_window, text="Dispense Drink", width=20)
    ...
    logout_button = tk.Button(menu_window, text="Logout", command=lambda: logout(menu_window))
...
name_label_main = tk.Label(root, text="Name:")
name_label_main.grid(row=0, column=0, padx=5, pady=5, sticky="e")
name_entry_main = tk.Entry(root, textvariable=name_var, state='readonly')
name_entry_main.grid(row=0, column=1, padx=5, pady=5, sticky="w")

balance_label_main = tk.Label(root, text="Balance:")
balance_label_main.grid(row=0, column=2, padx=5, pady=5, sticky="e")
balance_entry_main = tk.Entry(root, textvariable=balance_var, state='readonly')
...
for i, button in enumerate(buttons):
    ...
    col = i % 3
    if button == 'Clear':
        tk.Button(password_window, text=button, command=clear_password).grid(row=row, column=col, padx=5, pady=5)
    elif button == 'Enter':
        tk.Button(password_window, text=button, command=open_main_menu).grid(row=row, column=col, padx=5, pady=5)
    else:
        tk.Button(password_window, text=button, command=lambda b=button: enter_digit(b)).grid(row=row, column=col, padx=5, pady=5)
...
for i, option in enumerate(menu_options):
    row = i // 4 * 2 + 1
    col = i % 4
    button = tk.Button(root, text=option, command=lambda o=option: open_menu(o, name_var.get(), balance_var.get()), width=20, height=2)
